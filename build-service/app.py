...
/* Task 4 & 9: drill-down modal */
function openModal(r, isHistory){{
  ...
  const histRows = hist.map(x=>`
    <tr>
      ...
      ${{x.hasGap?'<td><span class="gap-badge">GAP</span></td>':'<td></td>'}}
    </tr>`).join('');
  const gapWarn = r.hasGap ? `<div class="gap-warn"><strong>GAP FLAG:</strong> A gap year was detected in this partner's filing history for this issuer. The consecutive count restarted after the gap. Human review required to confirm this was not an engineered reset.</div>` : '';
  document.getElementById('modal-title').textContent = `${{r.ep}} - ${{r.issuer}}`;
  document.getElementById('modal-body').innerHTML = `
    <div class="modal-section">
      <h3>Engagement Summary</h3>
      <div class="modal-grid">
        <span class="modal-label">Partner</span><span class="modal-value"><strong>${{r.ep}}</strong></span>
        <span class="modal-label">Issuer</span><span class="modal-value">${{r.issuer}}</span>
        ...
        <span class="modal-label">EQR</span><span class="modal-value">${{r.eqr?`<span class="eqr-biz">${{r.eqr}}</span>`:'Not in Bizinta'}}</span>
        <span class="modal-label">Signer</span><span class="modal-value">${{r.signer||'-'}}</span>
        <span class="modal-label">Status</span><span class="modal-value"><span class="pill ${{pc(r.sc)}}">${{slabel(r.sc)}}</span></span>
        <span class="modal-label">Years Remaining</span><span class="modal-value"><strong style="color:${{r.left==0?'#cc0000':r.left==1?'#cc5500':'#006600'}}">${{r.left==0?'ROTATE NOW':r.left+' year(s)'}}</strong></span>
      </div>
    </div>
    <div class="modal-section">
      <h3>Calculation Logic</h3>
      <div class="calc-box">${{r.calcNote}}</div>
      ${{gapWarn}}
    </div>
    <div class="modal-section">
...
function renderDash(data){{
  ...
  tb.innerHTML=data.map(r=>`<tr class="clickable${{r.hasGap?' gap-row':''}}" onclick="openModal(${{JSON.stringify(r).replace(/'/g,'\\u0027')}},false)">
    <td><strong>${{r.ep}}</strong></td><td>${{r.issuer}}${{r.hasGap?'<span class="gap-badge">GAP</span>':''}}</td>
...
function renderHist(data){{
  ...
  tb.innerHTML=data.map(r=>`<tr class="clickable${{r.hasGap?' gap-row':''}}" onclick="openModal(${{JSON.stringify(r).replace(/'/g,'\\u0027')}},true)">
